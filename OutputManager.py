...
class OutputManager:
	...
	def InitOutput(self, Window, OutCount, DeskBounds):
		#hr # HRESULT
		...
		for DriverTypeIndex in range(NumDriverTypes):
			hr = ctypes.windll.d3d11.D3D11CreateDevice(None, DriverType[DriverTypeIndex], None, 0, ctypes.byref((ctypes.c_uint * D3D11_SDK_VERSION)(*FeatureLevels)), NumFeatureLevels,
				D3D11_SDK_VERSION, ctypes.byref(self.__m_Device), ctypes.byref(FeatureLevel), ctypes.byref(self.__m_DeviceContext))
			if hr == 0:
